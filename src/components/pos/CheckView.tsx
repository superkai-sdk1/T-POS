...
export function CheckView({ onBack }: CheckViewProps) {
  ...
  const attachSpaceToCheck = useCallback(async (item: InventoryItem) => {
    if (!activeCheck || !item.linked_space_id || !item.linked_space) return;
    const now = new Date().toISOString();
    const spaceId = item.linked_space_id;
    const space = item.linked_space;
    // Update DB
  ...
  useEffect(() => {
    ...
    setHeader({
      ...
      showAvatar: true,
      playerId: activeCheck.player?.id || null,
      rightContent: (
        <button
  ...
  const handleAdd = (item: InventoryItem) => {
    hapticFeedback('light');
    // If item is linked to a space, attach it to the check instead of adding to cart
    if (item.linked_space_id && item.linked_space) {
      attachSpaceToCheck(item);
      return;
    }
    addToCart(item);
  };
        ...
        {cart.length === 0 ? (
          ...
          <div className="space-y-3 stagger-children">
            ...
            <button
              ...
              className="w-full p-4 rounded-[1.5rem] border-2 border-dashed border-white/10 hover:border-white/20 bg-white/[0.02] hover:bg-white/[0.04] transition-all flex items-center justify-center gap-2 text-white/40 hover:text-white/60 active:scale-[0.98]"
            >
              <Plus className="w-5 h-5" />
              <span className="text-sm font-bold uppercase tracking-wider">Добавить позицию</span>
            </button>
